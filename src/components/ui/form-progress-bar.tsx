...
export default function FormProgressBar({
  ...
  return (
    <div className="mx-2 my-4">
      <div className="mx-auto flex w-full sm:w-11/12 md:w-5/6 xl:w-3/4">
        {Array.from(Array(steps), (_, i) => i + 1).map((num: number) => (
          <Fragment key={num}>
            <div
              className={`relative flex h-10 w-10 shrink-0 overflow-hidden rounded-full border-2 ${
                num < position
            ...
            {num !== steps && (
              <div className="mx-1 flex h-10 grow flex-col sm:mx-2 md:mx-3 lg:mx-4">
                <span
                  className={`border-b ${
                    num < position
                      ? "border-b-green-500 dark:border-b-green-700"
                      : "border-b-slate-400 dark:border-b-slate-700"
                  } grow`}
                ></span>
                <span
                  className={`border-t ${
                    num < position
                      ? "border-t-green-500 dark:border-t-green-700"
                      : "border-t-slate-400 dark:border-t-slate-700"
                  } grow`}
                ></span>
              </div>
            )}
